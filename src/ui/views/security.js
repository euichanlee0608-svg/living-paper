...
export function render(root) {
    ...
    openSheet('노드 대면 확인', html`
      <div class="stack">
        <p class="tiny mut" style="line-height:1.7">
          ...
          <b style="color:var(--ink)">릴레이를 거치지 않는 경로</b>로 아래 숫자를 맞춰보는 것뿐입니다.
        ...
        <button class="btn btn-ghost btn-block" data-sheet-close>다릅니다 / 나중에</button>
      </div>`);
  });
  ...
  S.on(root, 'click', '[data-recovery]', async () => {
    ...
    openSheet('복구 키트', html`
      <div class="stack">
        ...
        ${fresh ? html`<div class="notice" style="background:var(--surface-2);border-color:var(--line);color:var(--body)">
        ...
        <button class="btn btn-ghost btn-block" onclick="window.print()">인쇄해서 보관</button>
      </div>`);
  });
  ...
  S.on(root, 'click', '[data-wipe]', () => {
    openSheet('이 기기에서 모두 삭제', html`
      <div class="stack">
        <div class="notice" style="background:var(--danger-bg);border-color:var(--danger-line);color:var(--danger)">
          ${raw(ico('alert'))}
        ...
        <button class="btn btn-ghost btn-block" data-sheet-close>취소</button>
      </div>`);
  });
  ...
  root.innerHTML = html`
    <div class="view">
      ...
      <div class="card card-pad">
        <div class="label">데이터가 지나가는 길</div>
        <div class="flow">
          <div class="flow-node">
          ...
          <div class="flow-node blind">
            ...
            <div class="d">전달·인증만<br><b style="color:var(--lock)">내용 열람 불가</b></div>
          </div>
          <div class="flow-arrow">${raw(ico('arrowRight'))}<span>암호문</span></div>
          <div class="flow-node">
        ...
        <p class="tiny mut" style="line-height:1.7">
          릴레이가 알 수 있는 것: <b style="color:var(--body)">누가 언제 몇 바이트를 보냈는지</b>.
      ...
      <div class="card card-pad" style="margin-top:var(--sp-4)">
        <div class="between" style="margin-bottom:var(--sp-4)">
          <div class="label" style="margin:0">내 기기 키</div>
        ...
        <div class="row wrap" style="margin-top:11px;gap:7px">
          ...
          <button class="btn btn-sm btn-ghost" data-copy-fp="${me?.fingerprint?.hex || ''}">
      ...
      <div class="card card-pad" style="margin-top:var(--sp-4)">
        <div class="between" style="margin-bottom:var(--sp-4)">
          <div class="label" style="margin:0">랩 노드</div>
          ${node?.verified
        ...
        ${node ? html`
          ...
          ${!node.verified ? html`
            <button class="btn btn-primary btn-block" data-verify style="margin-top:var(--sp-4)">
      ...
      <div class="card card-pad" style="margin-top:var(--sp-4)">
        <div class="between" style="margin-bottom:var(--sp-3)">
          <div class="label" style="margin:0">릴레이 감사 로그</div>
          <span class="tiny mut">${log.length}건</span>
        </div>
        <p class="tiny mut" style="margin-bottom:var(--sp-4);line-height:1.7">
      ...
      <!-- recovery + wipe -->
      <div class="card card-pad" style="margin-top:var(--sp-4)">
        <div class="label" style="margin-bottom:var(--sp-3)">복구와 삭제</div>
        <div class="stack">
          <button class="btn btn-ghost btn-block" data-recovery>${raw(ico('key'))} 복구 키트 보기</button>
          <button class="btn btn-danger btn-block" data-wipe>${raw(ico('trash'))} 이 기기에서 모두 삭제</button>
        </div>
      </div>

      <p class="tiny mut" style="margin-top:var(--sp-5);line-height:1.8;text-align:center">
        상세한 위협 모델과 한계는 저장소의
